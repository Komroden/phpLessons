    ...
    class CartM { 
        ...
        public function buy() {
            if (isset($_SESSION['cart_goods']) && isset($_SESSION['user_id'])) {
                ...
                $res = PdoM::Instance() -> Insert('orders', [
                    'id_user' => $userId, 
                    'date_order' => date("d-m-Y"), 
                    'status_order' => 'Активен']);

                $orderId = PdoM::Instance() -> lastIndex();
                ...
                foreach ($orderGood as $key => $val) {
                    ...
                    $goodSize = $val['size'];
                    $res = PdoM::Instance() -> Insert('order_goods', [
                        'id_order' => $orderId, 
                        'id_good' => $goodId, 
                        'good_count' => $goodCount, 
                        'good_size' => $goodSize]); 
                    
                    $orderCount = PdoM::Instance() -> Select("SELECT `order_count` FROM goods WHERE id=$goodId");
                    foreach($orderCount as $key => $val){
                        $res = PdoM::Instance() -> Update('goods', ['order_count'=>$val['order_count'] + 1], 'id = '.$goodId);
                    }
                }
                unset($_SESSION['cart_goods']);
            }        
